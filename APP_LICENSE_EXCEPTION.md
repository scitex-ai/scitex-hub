...
SciTeX Hub is licensed under the GNU Affero General Public License
version 3 (AGPL-3.0). This exception clarifies the licensing status of
third-party apps ("apps") that use the SciTeX App SDK.

## Exception Statement

As a special exception, the copyright holders of SciTeX Hub grant you
permission to create apps (workspace modules) that communicate with
SciTeX Hub solely through the App SDK interfaces listed below,
**without those apps being considered derivative works** of SciTeX Hub
under the AGPL-3.0.
...
1. **SDK-only interaction.** Your app communicates with SciTeX Hub
...
2. **No core modification.** Your app does not modify, monkey-patch,
   or replace any SciTeX Hub core module, middleware, or template.
...
4. **No circumvention.** Your app does not bypass security checks,
   authentication, or access controls provided by SciTeX Hub.

## What This Exception Does NOT Cover

- Modifications to SciTeX Hub itself (those remain AGPL-3.0)
- Apps that import or copy substantial portions of SciTeX Hub source
- Apps distributed as part of a modified SciTeX Hub distribution
