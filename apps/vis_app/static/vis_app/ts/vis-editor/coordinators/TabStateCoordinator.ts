...
import type { CanvasTabManager } from '../../vis/ui/CanvasTabManager.ts';
import type { DataTabManager } from '../../vis/ui/DataTabManager.ts';
import type { RulersManager } from '../../vis/RulersManager.ts';
...
export class TabStateCoordinator {
    ...
    public saveCanvasForCurrentTab(): void {
        ...
        const json = this.deps.canvasManager.canvas.toJSON([
            'name', 'id', 'axisMetadata', 'plotInfo',
            'originalWidth', 'originalHeight', 'csvData'
        ]);
        const viewState = {
    ...
    public async restoreCanvasForTab(tabId: string): Promise<void> {
        ...
        if (!hasCanvasContent || !this.deps.canvasManager.canvas) {
            ...
            const savedCanvasTheme = localStorage.getItem('canvas-theme') ||
            ...
            console.log(`[TabStateCoordinator] New tab or no content - canvas cleared, theme applied: ${savedCanvasTheme}`);
            return;
        }
        ...
        if (tabState.canvasJson) {
            ...
            return new Promise((resolve) => {
                this.deps.canvasManager.canvas!.loadFromJSON(tabState.canvasJson, () => {
                    ...
                    if (savedViewState) {
                        try {
                            ...
                            this.deps.canvasManager.setCanvasPanOffset(viewState.panX ?? 0, viewState.panY ?? 0);
                        } catch (e) {
                            if (tabState.viewState) {
                                this.deps.canvasManager.setCanvasZoomLevel(tabState.viewState.zoom);
                    ...
                    console.log(`[TabStateCoordinator] Restored canvas for tab ${tabId}`);
                    resolve();
                });
            });
        }
    ...
    public validateTabsAgainstFilesystem(): void {
        ...
        const removedFigureTabs = this.deps.canvasTabManager.validateAndCleanTabs(figzPaths);
        const validFigureIds = this.deps.canvasTabManager.getTabs().map(t => t.id);
        const removedDataTabs = this.deps.dataTabManager.validateAndCleanTabs(validFigureIds);

        if (removedFigureTabs > 0 || removedDataTabs > 0) {
            this.deps.updateStatusBar(`Cleaned up ${removedFigureTabs} figure(s) and ${removedDataTabs} table(s)`);
        }
    }
    ...
    public collectFigzPathsFromTree(): string[] {
        ...
        items.forEach(item => {
            ...
            if (path.endsWith('.figz')) paths.push(path);
        });
