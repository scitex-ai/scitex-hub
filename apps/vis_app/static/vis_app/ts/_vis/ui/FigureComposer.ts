...
export class FigureComposer {
  ...
  private renderPanelSlot(label: string): string {
    const position = this.layoutPositions[label] || {
      x: 0,
      y: 0,
      width: 1,
      height: 1,
  ...
  private renderPanelInfo(): string {
    ...
    const panel = this.currentFigure.panels.find(
      (p) => p.label === this.selectedPanelLabel,
    );
  ...
  private attachEventListeners(): void {
    // New figure button
    this.container
      .querySelector(".composer-new-btn")
      ?.addEventListener("click", () => {
        this.showNewFigureDialog();
      });

    this.container
      .querySelector(".composer-create-btn")
      ?.addEventListener("click", () => {
        this.showNewFigureDialog();
      });
    ...
    // Layout button
    this.container
      .querySelector(".composer-layout-btn")
      ?.addEventListener("click", () => {
    ...
    // Export button
    this.container
      .querySelector(".composer-export-btn")
      ?.addEventListener("click", () => {
  ...
  private selectPanel(label: string): void {
    ...
    const panel =
      this.currentFigure?.panels.find((p) => p.label === label) || null;
    this.options.onPanelSelect?.(label, panel);
  }

  private async addPanelFromBundle(
    label: string,
    pltzBundleId: string,
  ): Promise<void> {
    if (!this.currentFigure) return;
    ...
    try {
      const updatedFigure = await figzBundleManager.addPanel(
        this.currentFigure.id,
        {
          label,
  ...
  private async removePanel(label: string): Promise<void> {
    ...
    try {
      const updatedFigure = await figzBundleManager.removePanel(
        this.currentFigure.id,
        label,
      );
      this.currentFigure = updatedFigure;
  ...
  private async loadPanelPreviews(): Promise<void> {
    ...
    try {
      this.panelPreviews = await figzBundleManager.getPanelPreviews(
        this.currentFigure.id,
      );
    } catch (error) {
  ...
  private showLayoutDialog(): void {
    ...
    const layoutOptions: FigzLayout[] = [
      "1x1",
      "2x1",
      "1x2",
      "2x2",
  ...
  private showExportDialog(): void {
    ...
    const format = prompt("Export format (png, svg, pdf):")?.toLowerCase() as
      | "png"
      | "svg"
      | "pdf";
    ...
    const url = figzBundleManager.getPreviewUrl(
      this.currentFigure.id,
      format === "pdf" ? "png" : format,
    );
  ...
  public async loadFigure(figureId: string): Promise<void> {
    try {
      ...
      this.layoutPositions = await figzBundleManager.getLayoutPositions(
        this.currentFigure.layout,
      );
      await this.loadPanelPreviews();
   ...
   */
  public async handlePltzDrop(
    label: string,
    bundle: PltzBundleSummary,
  ): Promise<void> {
