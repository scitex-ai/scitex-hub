...
export class TreeInteraction {
  ...
  constructor(treeContainer: HTMLElement, callbacks: TreeInteractionCallbacks) {
  ...
  private setupTreeToggles(): void {
    ...
    this.treeContainer.addEventListener("click", (e) => {
      ...
      const header = target.closest(
        ".tree-item-header, .tree-group-header",
      ) as HTMLElement;

      if (
        toggle ||
        (header && !target.closest(".tree-action-btn, .tree-add-btn"))
      ) {
        e.stopPropagation();
  ...
  private handleTreeAction(btn: HTMLElement): void {
    ...
    const item = btn.closest(
      ".tree-item, .tree-group, .tree-section",
    ) as HTMLElement;
  ...
  private editItem(item: HTMLElement): void {
    ...
    else if (item.classList.contains("tree-annotation"))
      itemType = "annotation";

    console.log("[TreeInteraction] Edit requested:", {
      itemType,
      itemId,
      label,
    });
  ...
  private deleteItem(item: HTMLElement): void {
    ...
    console.log("[TreeInteraction] Delete requested:", {
      itemType,
      itemId,
      label,
    });
  ...
  private addItem(parent: HTMLElement): void {
    ...
    console.log("[TreeInteraction] Add requested:", {
      parentType,
      parentId,
      label,
      addableTypes,
  ...
  public selectItem(item: HTMLElement): void {
    ...
    const previousSelected = this.treeContainer.querySelector(
  ...
  private expandParents(item: HTMLElement): void {
    ...
    while (parent && parent !== this.treeContainer) {
      if (
        ...
        const parentItem = parent.closest(
          ".tree-item, .tree-group",
        ) as HTMLElement;
        if (parentItem) {
          ...
          const children = parentItem.querySelector(
            ".tree-item-children, .tree-group-children",
          ) as HTMLElement;

          if (
            toggle &&
            children &&
            toggle.classList.contains("fa-chevron-right")
          ) {
            // Expand if collapsed
