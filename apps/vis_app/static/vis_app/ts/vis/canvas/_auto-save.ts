...
export async function ensureFigzBundleExists(
  ...
  if (!state.projectOwner || !state.projectSlug) {
    console.warn(
      "[BundleCanvasManager] No project context - cannot create figz bundle",
    );
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        "[BundleCanvasManager] Failed to create figz bundle:",
        error,
      );
      return null;
    }
    ...
    console.log(
      "[BundleCanvasManager] Created figz bundle:",
      result.bundle_path,
    );
    return result.bundle_path || null;
  } catch (error) {
...
export async function triggerFigzAutoSave(
  ...
  const panelData: PanelData[] = panels.map((panel: any) => ({
    ...
    size: {
      width_mm:
        Math.round((panel.width || 80) * (panel.scaleX || 1) * pxToMm * 10) /
        10,
      height_mm:
        Math.round((panel.height || 68) * (panel.scaleY || 1) * pxToMm * 10) /
        10,
    },
  }));
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log(
        "[BundleCanvasManager] Figz bundle auto-saved:",
        result.bundle_path,
      );
