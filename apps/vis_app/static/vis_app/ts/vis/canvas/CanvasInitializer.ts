...
import { GridManager } from './GridManager.ts';
...
import { ZoomPanManager } from './ZoomPanManager.ts';
...
import { AlignmentManager } from './AlignmentManager.ts';
import { SnapManager } from './SnapManager.ts';
import { CropManager } from './CropManager.ts';
import { ElementSelectionManager } from './ElementSelectionManager.ts';
import { ContextMenuManager } from './ContextMenuManager.ts';
import { CanvasResizeManager } from './CanvasResizeManager.ts';
import { SessionManager } from './SessionManager.ts';
import { BundleCanvasManager } from './BundleCanvasManager.ts';
import { NudgeManager } from './NudgeManager.ts';
import { AxisDebugManager } from './AxisDebugManager.ts';
...
export interface InitCallbacks {
    ...
    onObjectResizedCallback?: (obj: any, newWidth: number, newHeight: number) => void;
    saveUndoState: () => void;
 ...
 */
export function initializeCanvas(callbacks: InitCallbacks): CanvasManagerRefs | null {
    ...
    if (!canvasElement) {
        console.error('[CanvasInitializer] Canvas element #vis-canvas not found in DOM');
        return null;
    }
    ...
    try {
        ...
        console.log(`[CanvasInitializer] Fabric.js canvas created in ${(canvasCreateTime - startTime).toFixed(2)}ms`);

        // Initialize all managers
        const gridManager = new GridManager(canvas, callbacks.statusBarCallback);
        const exportManager = new ExportManager(canvas, callbacks.statusBarCallback);
        const undoRedoManager = new UndoRedoManager(canvas, callbacks.statusBarCallback);
        const themeManager = new ThemeManager(canvas, initialIsDark, callbacks.statusBarCallback);
        const zoomPanManager = new ZoomPanManager(canvas, callbacks.rulersAreaTransformCallback, callbacks.statusBarCallback);
        const selectionManager = new SelectionManager(canvas, callbacks.statusBarCallback);
        ...
        const elementSelectionManager = new ElementSelectionManager(canvas, callbacks.statusBarCallback);
        ...
        const nudgeManager = new NudgeManager(canvas, callbacks.saveCanvasContent, callbacks.statusBarCallback);
        const axisDebugManager = new AxisDebugManager(canvas, callbacks.statusBarCallback);
        ...
        console.log(`[CanvasInitializer] ✅ Total init: ${(performance.now() - startTime).toFixed(2)}ms`);
...
export function setupCanvasEventListeners(
    ...
    const { canvas, snapManager, elementSelectionManager, bundleCanvasManager } = refs;
        ...
        if (target && target.type === 'group') {
            ...
            if (isPlotImage) {
                if (callbacks.statusBarCallback) {
                    callbacks.statusBarCallback('Plot data cannot be edited (scientific integrity)');
                }
                return;
            }
 ...
 */
function handleSelectionEvent(e: any, refs: CanvasManagerRefs, callbacks: InitCallbacks): void {
    ...
    if (callbacks.selectionCallback && e.selected && e.selected.length > 0) {
        ...
        if (e.selected.length === 1) {
            ...
            if ((selected.type === 'image' || selected.type === 'group') && selected.axisMetadata?.element_bboxes) {
                elementSelectionManager.enterElementSelectionMode(selected, { x: 0, y: 0 });
            } else {
