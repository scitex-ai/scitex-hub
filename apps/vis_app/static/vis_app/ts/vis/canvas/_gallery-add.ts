...
async function loadEmbeddedPanel(
    ...
    const previewUrl = `/vis/api/bundles/figz/panel-preview/?path=${encodeURIComponent(figzPath)}&panel=${encodeURIComponent(panelLabel)}&project_owner=${encodeURIComponent(state.projectOwner)}&project_slug=${encodeURIComponent(state.projectSlug)}&t=${Date.now()}`;
    ...
    try {
        ...
        console.log(`[BundleCanvasManager] ✓ Panel ${panelLabel} added (embedded in figz)`);

    } catch (error) {
        console.error(`[BundleCanvasManager] ✗ Failed to load embedded panel ${panelLabel}:`, error);
    }
}
...
export async function addPanelFromGallery(
    ...
    try {
        ...
        const response = await fetch('/vis/api/bundles/figz/add-panel/', {
            method: 'POST',
            headers: {
        ...
        console.log(`[BundleCanvasManager] Panel ${nextLabel} embedded in figz: ${figzPath}`);
        ...
        // Load the embedded panel onto canvas
