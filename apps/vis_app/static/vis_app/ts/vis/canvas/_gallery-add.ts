...
async function loadEmbeddedPanel(
  ...
  try {
    ...
    const hitmapUrl = `/vis/api/bundles/figz/panel-preview/?path=${encodeURIComponent(figzPath)}&panel=${encodeURIComponent(panelLabel)}&project_owner=${encodeURIComponent(state.projectOwner)}&project_slug=${encodeURIComponent(state.projectSlug)}&type=hitmap`;
