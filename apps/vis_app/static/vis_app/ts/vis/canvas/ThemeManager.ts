...
export class ThemeManager {
  ...
  constructor(
    ...
    console.log(
      `[ThemeManager] Initialized in ${isDarkMode ? "dark" : "light"} mode`,
    );
  }
  ...
  public updateCanvasTheme(
    ...
    this.canvas.backgroundColor = isDark ? "#2a2a2a" : "#fdfcfa";
    ...
    console.log(
      `[ThemeManager] Canvas theme updated to ${isDark ? "dark" : "light"} mode`,
    );
  }
  ...
  public processImageForDarkMode(img: HTMLImageElement): string {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      if (
        r < this.BLACK_THRESHOLD &&
        g < this.BLACK_THRESHOLD &&
        b < this.BLACK_THRESHOLD
      ) {
  ...
  public updateImageForTheme(fabricImg: any): void {
    ...
    console.log(
      `[ThemeManager] updateImageForTheme: originalSrc=${originalSrc?.substring(0, 80)}...`,
    );
    ...
    if (
      ...
      console.warn(
        `[ThemeManager] Skipping image with invalid source: ${originalSrc}`,
      );
      return;
    }
    ...
    if (this.isDarkMode) {
      ...
      tempImg.onload = () => {
        console.log(
          `[ThemeManager] Original image loaded, processing for dark mode...`,
        );
        const processedSrc = this.processImageForDarkMode(tempImg);
        console.log(
          `[ThemeManager] Dark mode processing complete, src length=${processedSrc.length}`,
        );
      ...
      tempImg.onerror = (err) => {
        console.error(
          `[ThemeManager] Failed to load original image for dark mode`,
          err,
        );
      };
  ...
  public reprocessAllImagesForTheme(): void {
    ...
    if (processedCount > 0) {
      console.log(
        `[ThemeManager] Reprocessed ${processedCount} images for ${this.isDarkMode ? "dark" : "light"} mode`,
      );
    }
  }
  ...
  public processNewImage(fabricImg: any): void {
    console.log(
      `[ThemeManager] processNewImage called, isDarkMode=${this.isDarkMode}, obj.type=${fabricImg?.type}, isBundlePanel=${fabricImg?.isBundlePanel}`,
    );
    ...
    if (this.isDarkMode) {
      ...
      console.log(
        `[ThemeManager] Skipping dark mode processing (light mode active)`,
      );
    }
  }
  ...
  private isDarkColor(color: string | null | undefined): boolean {
    ...
    return this.DARK_COLORS.some(
      (dark) => normalizedColor === dark.toLowerCase().replace(/\s/g, ""),
    );
  }
  ...
  public processSvgGroupForDarkMode(group: any): void {
    ...
    console.log(
      `[ThemeManager] Dark mode: modified ${modifiedCount} path colors in group`,
    );
  }
  ...
  public restoreSvgGroupColors(group: any): void {
    ...
    console.log(
      `[ThemeManager] Light mode: restored ${modifiedCount} path colors in group`,
    );
  }
  ...
  public reprocessAllSvgGroupsForTheme(): void {
    ...
    this.canvas.renderAll();
    console.log(
      `[ThemeManager] Reprocessed ${groupsToProcess.length} SVG groups for ${this.isDarkMode ? "dark" : "light"} mode`,
    );
  }
