...
def tool_render_md(request):
    ...
    return render(request, "public_app/tools/render-md.html", _tool_context(request))


def tool_diff_texts(request):
    """Text Diff Checker tool detail page."""
    return render(request, "public_app/tools/diff-texts.html", _tool_context(request))


def tool_format_json(request):
    """JSON Formatter tool detail page."""
    return render(request, "public_app/tools/format-json.html", _tool_context(request))


# Image tools
def tool_view_image(request):
    """Image Viewer - Display image with dimension, DPI, and unit conversion info."""
    return render(request, "public_app/tools/view-image.html", _tool_context(request))


def tool_resize_image(request):
    """Image Resizer tool detail page."""
    return render(request, "public_app/tools/resize-image.html", _tool_context(request))


def tool_crop_images(request):
    """Image Cropper tool detail page."""
    return render(request, "public_app/tools/crop-images.html", _tool_context(request))


def tool_convert_image_format(request):
    """Image Converter tool detail page."""
    return render(
        request, "public_app/tools/convert-image-format.html", _tool_context(request)
    )
...
def tool_concat_images(request):
    ...
    return render(
        request, "public_app/tools/concat-images.html", _tool_context(request)
    )


def tool_render_mmd(request):
    """Mermaid Diagram Renderer - Create diagrams from text syntax."""
    return render(request, "public_app/tools/render-mmd.html", _tool_context(request))


def tool_convert_images_to_gif(request):
    """Images to GIF tool detail page."""
    return render(
        request, "public_app/tools/convert-images-to-gif.html", _tool_context(request)
    )
...
def tool_convert_images_to_pdf(request):
    ...
    return render(
        request, "public_app/tools/convert-images-to-pdf.html", _tool_context(request)
    )
...
def tool_convert_pdf_to_images(request):
    ...
    return render(
        request, "public_app/tools/convert-pdf-to-images.html", _tool_context(request)
    )
...
def tool_merge_pdf(request):
    ...
    return render(request, "public_app/tools/merge-pdf.html", _tool_context(request))


def tool_compress_pdf(request):
    """PDF Compressor tool detail page."""
    return render(request, "public_app/tools/compress-pdf.html", _tool_context(request))


def tool_split_pdf(request):
    """PDF Splitter tool detail page."""
    return render(request, "public_app/tools/split-pdf.html", _tool_context(request))


# Video tools
def tool_edit_video(request):
    """Video Editor tool detail page."""
    return render(request, "public_app/tools/edit-video.html", _tool_context(request))


# Rendering tools
def tool_view_plot(request):
    """Quick CSV Plot Viewer - renders simple CSV plots using Canvas."""
    return render(request, "public_app/tools/view-plot.html", _tool_context(request))


def tool_test_scitex_plot(request):
    """Backend Plot Renderer Test - test matplotlib/scitex.plt backend."""
    return render(
        request, "public_app/tools/test-scitex-plot.html", _tool_context(request)
    )


def tool_pick_color(request):
    """Color Picker tool detail page."""
    return render(request, "public_app/tools/pick-color.html", _tool_context(request))
...
def tool_inspect_html_element(request):
    ...
    return render(
        request, "public_app/tools/inspect-html-element.html", _tool_context(request)
    )


def tool_concat_repo(request):
    """Repository Concatenator tool detail page."""
    return render(request, "public_app/tools/concat-repo.html", _tool_context(request))


def tool_generate_qr(request):
    """QR Code Generator tool detail page."""
    return render(request, "public_app/tools/generate-qr.html", _tool_context(request))
...
def tool_scrape_citations(request):
    ...
    return render(
        request, "public_app/tools/scrape-citations.html", _tool_context(request)
    )


def tool_run_stats(request):
    """Statistics Calculator tool detail page."""
    return render(request, "public_app/tools/run-stats.html", _tool_context(request))
...
def tool_convert_docx_to_latex(request):
    ...
    return render(
        request,
        "public_app/tools/convert-docx-to-latex.html",
