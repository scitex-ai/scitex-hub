/**
 * Citation Upload Module
 * Handles .bib file upload and drag-drop functionality
 */

import { getCsrfToken } from "../../shared/utils";

export class CitationUpload {
  ...
  constructor(projectId: string | null = null) {
    ...
    console.log("[CitationUpload] Initialized with project:", projectId);
  }
  ...
  /**
   * Setup drop zone for .bib file uploads
   */
  setupDropZone(
    dropZoneId: string = "citations-drop-zone",
    fileInputId: string = "citations-file-input",
  ): void {
    const dropZone = document.getElementById(dropZoneId);
    const fileInput = document.getElementById(fileInputId) as HTMLInputElement;

    if (!dropZone || !fileInput) {
      console.warn("[CitationUpload] Drop zone elements not found");
      return;
    }
    ...
    console.log("[CitationUpload] Drop zone setup complete");
  }
  ...
  private async handleFileUpload(files: File[]): Promise<void> {
    if (!this.projectId) {
      console.error("[CitationUpload] No project ID for upload");
    ...
    try {
      ...
      if (data.success) {
        console.log("[CitationUpload] Upload successful:", data);
        alert(
          `Successfully uploaded ${files.length} bibliography file(s). ${data.entries_added || 0} new entries added.`,
        );
        this.onUploadSuccess?.();
      } else {
        console.error("[CitationUpload] Upload failed:", data);
        alert(`Upload failed: ${data.error || "Unknown error"}`);
      }
    } catch (error) {
      console.error("[CitationUpload] Upload error:", error);
