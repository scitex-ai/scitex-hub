...
import { getWriterFilter, WriterFileFilter } from "./_writer-file-filter";
import { FileTreeNode, FileTreeOptions } from "./_file-tree/types";
import { TreeRenderer } from "./_file-tree/tree-renderer";
import { DirectoryManager } from "./_file-tree/directory-manager";
...
export class FileTreeManager {
  ...
  async load(): Promise<void> {
    try {
      ...
      const response = await this.apiClient.get<{ tree: FileTreeNode[] }>(
        `/apps/writer/api/project/${this.projectId}/file-tree/`,
      );
  ...
  private focusOnSection(section: string): void {
    ...
    const expectedPath = this.filter.getExpectedFilePath(
      currentDoctype,
      section,
    );

    console.log(
      "[FileTree] Attempting to focus on section file:",
      expectedPath,
    );
