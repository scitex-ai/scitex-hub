...
def parse_query_operators(query):
    ...
    Delegates to scitex.scholar SearchQueryParser.from_shell_syntax().
    ...
    parser = SearchQueryParser.from_shell_syntax(query)
    filters = parser.get_filters()

    # Map to the legacy dict format expected by callers
    return {
        "query": parser.get_keyword_query(),
        "title_includes": filters.get("title_includes", []),
        "title_excludes": filters.get("title_excludes", []),
        "author_includes": filters.get("author_includes", []),
        "author_excludes": filters.get("author_excludes", []),
        "journal_includes": filters.get("journal_includes", []),
        "journal_excludes": filters.get("journal_excludes", []),
        "year_min": filters.get("year_start"),
        "year_max": filters.get("year_end"),
        "citations_min": filters.get("min_citations"),
        "citations_max": filters.get("max_citations"),
        "impact_factor_min": filters.get("min_impact_factor"),
        "impact_factor_max": filters.get("max_impact_factor"),
    }
...
def apply_advanced_filters(results, filters, parsed_operators=None):
    ...
    Delegates to scitex.scholar.filters.apply_filters().
    """
    from scitex.scholar.filters import apply_filters

    return apply_filters(results, filters=filters, parsed_operators=parsed_operators)
