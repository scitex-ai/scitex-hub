...
class PDFDownloadManager {
    ...
    async downloadSelected() {
        const selectedCards = document.querySelectorAll(".result-card .paper-select:checked, .result-card-compact .paper-select-checkbox:checked");
        let success = 0;
        let failed = 0;
...
function scheduleInitialization() {
    ...
    const scheduleCallback = window.requestIdleCallback || window.requestAnimationFrame;
    scheduleCallback(() => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const existingBadges = document.querySelectorAll(".pdf-status-badge[data-status='unknown']");
    ...
    if (resultsContainer) {
            ...
            for (const mutation of mutations) {
                for (const node of mutation.addedNodes) {
                    if (node instanceof HTMLElement) {
                        ...
                        if (node.classList.contains("pdf-status-badge") &&
        ...
        console.log("[PDF Download] MutationObserver setup with deferred initialization");
    }
