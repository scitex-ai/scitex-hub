...
export function updateLimitInfo(
  ...
  for (const li of limitInfoChain) {
    if (li.limit_reason) {
      ...
      tooltipParts.push(
        `${li.stage}: ${formatNumber(li.returned)} of ${formatNumber(li.total_available)} (limit=${li.configured_limit || "?"})`,
      );
    }
  }
  ...
  if (resultCount && resultCount > RENDER_LIMIT) {
    tooltipParts.push(
      `Browser: Showing ${RENDER_LIMIT} of ${formatNumber(resultCount)} fetched`,
    );
  }
 ...
 */
export function clearLimitInfo(): void {
  const limitInfoEl = document.getElementById("resultsLimitInfo");
  if (limitInfoEl) {
