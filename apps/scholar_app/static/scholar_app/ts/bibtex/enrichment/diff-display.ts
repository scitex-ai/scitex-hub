 ...
 * @module diff-display
 */
import { updateElementHTML } from "./ui-utils";
/**
 * Show BibTeX diff modal
 */
export async function showBibtexDiff(jobId) {
    if (!jobId) {
    ...
    try {
        ...
        if (data.success) {
            ...
            updateElementHTML("bibtexDiffContent", `<div style="color: var(--error-color);">Error: ${data.error || "Failed to generate diff"}</div>`);
        }
    }
    catch (error) {
        console.error("Error fetching diff:", error);
        updateElementHTML("bibtexDiffContent", `<div style="color: var(--error-color);">Failed to load comparison: ${error.message}</div>`);
    }
}
/**
 * Display the diff in a readable format (GitHub-style)
 */
function displayBibtexDiff(diffData, stats, files) {
    let html = "";
 ...
 */
function buildFileComparisonSection(files) {
 ...
 */
function buildFileLink(filename, url) {
    if (!url) {
...
function buildStatisticsSection(stats) {
    ...
    // Enhancement rate
    html += buildStatCard("Enhancement Rate", `${stats.enhancement_rate}%`, "#0366d6");
    html += `</div></div>`;
    return html;
}
/**
 ...
 */
function buildMajorFieldsSection(stats) {
    if (!stats || !stats.major_fields)
        return "";
    ...
    // Citation Count
    html += buildFieldCard("CITATION COUNT", stats.major_fields.citation_count, stats.total_entries);
    // Impact Factor
    html += buildFieldCard("IMPACT FACTOR", stats.major_fields.impact_factor, stats.total_entries);
    html += `</div></div>`;
    return html;
}
/**
 ...
 */
function buildFieldCard(fieldName, fieldData, totalEntries) {
 ...
 */
function buildNoChangesSection() {
 ...
 */
function buildDiffEntries(diffData) {
    let html = "";
 ...
 */
function buildAddedFields(fields) {
    let html = `<div style="background: #e6ffed; padding: 1rem;">`;
    fields.forEach((field) => {
        const fieldValue = field.value.length > 100
 ...
 */
function buildChangedFields(fields) {
    let html = `<div style="background: #fff5b1; padding: 1rem;">`;
    fields.forEach((field) => {
        const oldValue = field.old_value.length > 80
            ? field.old_value.substring(0, 80) + "..."
            : field.old_value;
        const newValue = field.new_value.length > 80
 ...
 */
function buildUnchangedFields(fields) {
    ...
    fields.forEach((field) => {
        const fieldValue = field.value.length > 80
 ...
 */
export function closeBibtexDiff() {
    const modal = document.getElementById("bibtexDiffModal");
    if (modal)
 ...
 */
export function toggleProcessingLogVisibility() {
...
//# sourceMappingURL=diff-display.ts.map
