...
import { ForceSimulation } from "./_ForceSimulation";
...
export class GraphRenderer {
  ...
  private resolveEdges(
    ...
    return edges
      .map((e) => ({
        ...e,
        source:
          typeof e.source === "string" ? nodeMap.get(e.source)! : e.source,
        target:
          typeof e.target === "string" ? nodeMap.get(e.target)! : e.target,
      }))
  ...
  private createElements(
    ...
    edges.forEach((edge) => {
      ...
      line.setAttribute(
        "stroke-width",
        String(Math.max(1, Math.min(edge.weight / 20, 3))),
      );
  ...
  public filterEdges(activeTypes: Set<string>): void {
    if (!this.svg) return;
    this.svg.querySelectorAll(".graph-edge").forEach((edge) => {
      const classList = edge.getAttribute("class") || "";
      const isVisible =
        activeTypes.size === 0 ||
        Array.from(activeTypes).some((t) => classList.includes(`edge-${t}`));
      (edge as SVGElement).style.display = isVisible ? "" : "none";
    });
  }
}
