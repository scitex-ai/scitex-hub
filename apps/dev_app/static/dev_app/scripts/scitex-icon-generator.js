/**
 * SciTeX Icon Generator - Interactive SVG generator for the design system
 */
...
(function () {
  ...
  const state = {
    snakeColor: "#ffffff",
    snakeCamoEnabled: false,
    snakeCamoStyle: "Forest",
    bgColor: "#1a2a40",
    bgTransparent: false,
    bgCamoEnabled: false,
    bgCamoStyle: "Forest",
    textEnabled: false,
    textContent: "scitex.ai",
    textColor: "#ffffff",
    textPosition: "bottom",
  };

  /**
   * Generate SVG content based on current state
   */
  function generateSVG() {
    ...
    // Snake camo pattern
    if (state.snakeCamoEnabled) {
      const colors = CAMO_PALETTES[state.snakeCamoStyle];
      svgParts.push("  <defs>");
      hasDefs = true;
      svgParts.push(
        '    <pattern id="snake-camo" patternUnits="userSpaceOnUse" width="200" height="200">'
      );
      svgParts.push(`      <rect width="200" height="200" fill="${colors[0]}"/>`);
      svgParts.push(`      <ellipse cx="50" cy="30" rx="60" ry="40" fill="${colors[1]}"/>`);
      svgParts.push(`      <ellipse cx="150" cy="80" rx="50" ry="35" fill="${colors[2]}"/>`);
      svgParts.push(`      <ellipse cx="30" cy="120" rx="45" ry="30" fill="${colors[3]}"/>`);
      svgParts.push(`      <ellipse cx="120" cy="160" rx="55" ry="38" fill="${colors[1]}"/>`);
      svgParts.push("    </pattern>");
    }

    // Background camo pattern
    if (!state.bgTransparent && state.bgCamoEnabled) {
      const colors = CAMO_PALETTES[state.bgCamoStyle];
      if (!hasDefs) {
        svgParts.push("  <defs>");
        hasDefs = true;
      }
      svgParts.push(
        '    <pattern id="bg-camo" patternUnits="userSpaceOnUse" width="200" height="200">'
      );
      svgParts.push(`      <rect width="200" height="200" fill="${colors[0]}"/>`);
      svgParts.push(`      <ellipse cx="50" cy="30" rx="60" ry="40" fill="${colors[1]}"/>`);
      svgParts.push(`      <ellipse cx="150" cy="80" rx="50" ry="35" fill="${colors[2]}"/>`);
    ...
    if (!state.bgTransparent) {
      if (state.bgCamoEnabled) {
        svgParts.push('  <circle cx="1080" cy="1080" r="1080" fill="url(#bg-camo)"/>');
      } else {
        svgParts.push(`  <circle cx="1080" cy="1080" r="1080" fill="${state.bgColor}"/>`);
      }
    }

    // Snake fill
    const snakeFill = state.snakeCamoEnabled ? "url(#snake-camo)" : state.snakeColor;

    // Snake paths
    svgParts.push("  <g>");
    for (const path of SNAKE_PATHS) {
      svgParts.push(`    <path fill="${snakeFill}" d="${path}"/>`);
    }
    svgParts.push("  </g>");

    // Text
    if (state.textEnabled && state.textContent) {
      const yPos = { top: 400, center: 1080, bottom: 1800 }[state.textPosition];
      svgParts.push(`  <text x="1080" y="${yPos}" text-anchor="middle" `);
      svgParts.push('        font-family="Arial, sans-serif" font-size="180" font-weight="bold" ');
      svgParts.push(`        fill="${state.textColor}">${escapeHtml(state.textContent)}</text>`);
    }
  ...
  function updateFilename() {
    const parts = ["scitex-icon"];

    // Snake part
    if (state.snakeCamoEnabled) {
      parts.push(`camo-${state.snakeCamoStyle.toLowerCase()}`);
    } else {
      for (const [name, hex] of Object.entries(COLOR_OPTIONS)) {
        if (hex === state.snakeColor) {
          parts.push(name.toLowerCase().replace(" ", "-"));
          break;
        }
      }
    }
    ...
    } else if (state.bgCamoEnabled) {
      parts.push(`camo-${state.bgCamoStyle.toLowerCase()}`);
    } else {
      for (const [name, hex] of Object.entries(COLOR_OPTIONS)) {
        if (hex === state.bgColor) {
          parts.push(name.toLowerCase().replace(" ", "-"));
          break;
        }
      }
    ...
    const filenameEl = document.getElementById("icon-filename");
    if (filenameEl) {
  ...
  function updatePreview() {
    ...
    // Remove XML declaration for inline display
    const inlineSvg = svg.replace(/<\?xml[^?]*\?>\n?/, "");
    previewEl.innerHTML = inlineSvg;
  ...
  function createColorPickers() {
    ...
    for (const [containerId, callback] of Object.entries(containers)) {
      ...
      for (const [name, hex] of Object.entries(COLOR_OPTIONS)) {
        ...
        btn.className = "scitex-icon-gen-color-btn";
        btn.style.backgroundColor = hex;
        btn.title = name;
        btn.dataset.color = hex;

        // Add border for light colors
        if (["#ffffff", "#d1d5db", "#e8e8e8"].includes(hex)) {
          btn.style.border = "1px solid #666";
        }

        btn.addEventListener("click", () => {
          // Update active state
          container.querySelectorAll(".scitex-icon-gen-color-btn").forEach((b) => {
      ...
      const defaultColor =
        containerId === "snake-colors"
          ? state.snakeColor
          : containerId === "bg-colors"
            ? state.bgColor
            : state.textColor;
      const activeBtn = container.querySelector(`[data-color="${defaultColor}"]`);
      if (activeBtn) {
  ...
  function setupEventListeners() {
    // Snake camo toggle
    const snakeCamoToggle = document.getElementById("snake-camo-toggle");
    const snakeCamoStyle = document.getElementById("snake-camo-style");
    if (snakeCamoToggle && snakeCamoStyle) {
      snakeCamoToggle.addEventListener("change", (e) => {
        state.snakeCamoEnabled = e.target.checked;
        snakeCamoStyle.disabled = !e.target.checked;
        updatePreview();
      });
      snakeCamoStyle.addEventListener("change", (e) => {
        state.snakeCamoStyle = e.target.value;
        updatePreview();
      });
    }
    ...
    if (bgTransparentToggle) {
      bgTransparentToggle.addEventListener("change", (e) => {
        state.bgTransparent = e.target.checked;
        updatePreview();
      });
    }

    // Background camo toggle
    const bgCamoToggle = document.getElementById("bg-camo-toggle");
    const bgCamoStyle = document.getElementById("bg-camo-style");
    if (bgCamoToggle && bgCamoStyle) {
      bgCamoToggle.addEventListener("change", (e) => {
        state.bgCamoEnabled = e.target.checked;
        bgCamoStyle.disabled = !e.target.checked;
        updatePreview();
      });
      bgCamoStyle.addEventListener("change", (e) => {
        state.bgCamoStyle = e.target.value;
        updatePreview();
      });
    }
    ...
    if (textToggle) {
      textToggle.addEventListener("change", (e) => {
        state.textEnabled = e.target.checked;
        if (textContent) textContent.disabled = !e.target.checked;
        if (textPosition) textPosition.disabled = !e.target.checked;
        updatePreview();
      });
    }
    if (textContent) {
    ...
    if (textPosition) {
      textPosition.addEventListener("change", (e) => {
        state.textPosition = e.target.value;
        updatePreview();
      });
    }
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener("click", () => {
        ...
        a.href = url;
        a.download = document.getElementById("icon-filename")?.textContent || "scitex-icon.svg";
   ...
   */
  function init() {
    const container = document.getElementById("scitex-icon-generator");
    if (!container) return;
