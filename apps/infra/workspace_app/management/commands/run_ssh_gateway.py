...
from apps.infra.workspace_app.services.container_manager import UserContainerManager
from apps.infra.workspace_app.services.ssh_key_manager import SSHKeyManager
...
class SSHGateway(paramiko.ServerInterface):
    ...
    def check_auth_publickey(self, username: str, key: paramiko.PKey) -> int:
        ...
        try:
            ...
            logger.error(
                f"Public key authentication error for user {username}: {e}",
                exc_info=True,
            )
            return paramiko.AUTH_FAILED
    ...
    try:
        ...
        if "Error reading SSH protocol banner" in error_msg:
            if addr[0] == "127.0.0.1":
                ...
                logger.warning(
                    f"Connection from {addr[0]} closed without SSH handshake"
                )
        else:
