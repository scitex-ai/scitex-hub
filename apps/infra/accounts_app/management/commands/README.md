...
```bash
# Production
docker exec scitex-hub-prod-django-1 python manage.py list_users

# Development
docker exec scitex-hub-dev-django-1 python manage.py list_users
```
...
docker exec scitex-hub-prod-django-1 python manage.py list_users --staff-only --verbose

# Export all users as JSON
docker exec scitex-hub-prod-django-1 python manage.py list_users --format json > users.json

# List inactive users only
docker exec scitex-hub-prod-django-1 python manage.py list_users --inactive-only
```
