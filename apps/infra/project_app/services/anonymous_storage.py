...
# Configuration
VISITOR_STORAGE_ROOT = getattr(settings, "VISITOR_STORAGE_ROOT", "/tmp/scitex_visitor")
...
def get_session_info(session_key: str) -> Optional[dict]:
    ...
    """
    try:
        metadata_path = os.path.join(VISITOR_STORAGE_ROOT, session_key, "metadata.json")
        if os.path.exists(metadata_path):
            with open(metadata_path, "r") as f:
                return json.load(f)
        return None
