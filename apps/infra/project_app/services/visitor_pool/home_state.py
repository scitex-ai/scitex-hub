...
Companion module ``home_access`` owns the other question — the unix identity
the tree is handed back to and whether the app can reach it (ownership, mode,
and the gate for both). It imports :class:`HomeStateError` from here, so the
...
def visitor_home_root(visitor_user: User) -> Path:
    """The visitor's HOME root — the parent of ``proj/``, not ``proj/`` itself.

    Apptainer binds this directory as the container ``--home``
    (``scitex_container --home <home_root>:/home/<username>``), so it is the
    unit the wipe, the ownership hand-back and the mode all apply to. Derived
    through ``get_project_filesystem_manager`` rather than rebuilt from
    ``settings.BASE_DIR`` so there is exactly one definition of where a
    visitor's files live.
    """
    from apps.infra.project_app.services.project_filesystem import (
        get_project_filesystem_manager,
    )

    return Path(get_project_filesystem_manager(visitor_user).base_path).parent
