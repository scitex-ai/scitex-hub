    ...
    try:
        ...
        for line in result.stdout.splitlines():
            ...
            statuses[filepath] = GitFileStatus(
                path=filepath, status=status, staged=staged
            )
...
def parse_diff_output(diff_output: str) -> List[LineDiff]:
    ...
    while i < len(lines):
        ...
        if line.startswith("+") and not line.startswith("+++"):
            # Added line
            diffs.append(LineDiff(line_number=current_new_line, status="added"))
            current_new_line += 1
        elif line.startswith("-") and not line.startswith("---"):
            # Deleted line (mark the line before)
            diffs.append(LineDiff(line_number=current_new_line, status="deleted"))
    ...
    while i < len(diffs):
        ...
        if i + 1 < len(diffs):
            ...
            if (
                ...
                and next_diff.status == "added"
                and abs(next_diff.line_number - current.line_number) <= 1
            ):

                merged.append(
                    LineDiff(line_number=next_diff.line_number, status="modified")
                )
                i += 2
                continue
    ...
    def annotate_tree(items: List[dict], prefix: str = "") -> List[dict]:
        ...
        for item in items:
            ...
            if full_path in statuses:
                ...
                git_status = {"status": status_obj.status, "staged": status_obj.staged}

            # Create annotated item
            annotated_item = {**item, "git_status": git_status}

            # Recursively annotate children
            if item.get("children"):
                annotated_item["children"] = annotate_tree(item["children"], full_path)
