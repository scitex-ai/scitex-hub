...
class PortProxyWebSocketConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not self.user or not self.user.is_authenticated:
            logger.warning(
                f"Unauthenticated WebSocket connection attempt to {self.username}/{self.slug}"
            )
        ...
        try:
            ...
            self.project = await Project.objects.select_related("owner").aget(
                owner__username=self.username, slug=self.slug
            )

            # Check if user has access
            has_access = (
                self.project.owner_id == self.user.id
                or self.project.is_public
                or await self.project.collaborators.filter(id=self.user.id).aexists()
            )

            if not has_access:
                logger.warning(
                    f"User {self.user.username} denied access to {self.username}/{self.slug}"
                )
        ...
        try:
            ...
            self.target_ws = await websockets.connect(self.target_uri, **connect_kwargs)
