...
def api_extract_bundle(request, username, slug):
    ...
    if not bundle_path or not output_path:
        return JsonResponse(
            {
                "success": False,
                "error": "Both bundle_path and output_path are required",
            },
            status=400,
        )
    ...
    if not zipfile.is_zipfile(bundle_full):
        return JsonResponse(
            {
                "success": False,
                "error": f"Invalid bundle file (not a zip): {bundle_path}",
            },
            status=400,
        )
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": f"Invalid bundle archive: {str(e)}"}, status=400
        )
    except OSError as e:
