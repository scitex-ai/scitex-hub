    ...
    try:
        ...
        active_mounts = set(
            RemoteProjectConfig.objects.filter(
                is_mounted=True, mount_point__isnull=False
            ...
            for mount_dir in user_dir.iterdir():
                ...
                if mount_path not in active_mounts:
                    ...
                    try:
                        ...
                        stats["errors"] += 1
                        logger.warning(
                            f"⚠️ Failed to unmount stale mount {mount_path}: {e}"
                        )
                    except Exception as e:
                        stats["errors"] += 1
        ...
        logger.error(
            f"Unexpected error in cleanup stale mounts task: {e}", exc_info=True
        )
        raise
