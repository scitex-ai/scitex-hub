...
The main config/urls.py includes this at: path('<str:username>/', include('apps.infra.project_app.urls'))
"""
...
urlpatterns = [
    ...
    path("<slug:slug>/pulls/", include("apps.infra.project_app.urls.pull_requests")),
    ...
    # Issues feature
    path("<slug:slug>/issues/", include("apps.infra.project_app.urls.issues")),
    # Security feature
    path("<slug:slug>/security/", include("apps.infra.project_app.urls.security")),
    # Workflows feature (placeholder for now)
    # path('<slug:slug>/actions/', include('apps.infra.project_app.urls.workflows')),
    # Project management (settings, edit, delete)
    path("<slug:slug>/", include("apps.infra.project_app.urls.projects")),
    # Social actions (watch, star, fork)
    path("<slug:slug>/api/", include("apps.infra.project_app.urls.actions")),
    # Repository browsing (must be last as it has catch-all patterns)
    path("<slug:slug>/", include("apps.infra.project_app.urls.repository")),
]
