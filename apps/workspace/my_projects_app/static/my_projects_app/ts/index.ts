...
import { submitToAppStore } from "../../../../../infra/project_app/static/project_app/ts/shared/project-app/project-actions";
import {
  switchHubTab,
  loadHubTabContent,
  loadHubBrowse,
...
function initHub(): void {
  ...
  hubMain.addEventListener("click", (e: Event) => {
    ...
    if (topicsSave) {
      ...
      fetch("/apps/my-projects/api/update-topics/", {
        method: "POST",
        headers: {
...
window.addEventListener("popstate", (event) => {
  ...
  if (state?.view === "project" && state.owner && state.slug) {
    ...
    hubPost("/apps/my-projects/api/select-project/", {
      owner: state.owner,
      slug: state.slug,
    }).then((data) => {
      if (data?.success) {
    ...
    setModeActive("me");
    hubGet("/apps/my-projects/api/me/").then((data) => {
