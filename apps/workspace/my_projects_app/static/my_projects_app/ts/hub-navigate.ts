...
export async function selectProject(projectId: string): Promise<void> {
  ...
  const data = await hubPost("/apps/my-projects/api/select-project/", {
...
export async function loadExplore(tab: string): Promise<void> {
  ...
  const data = await hubGet(
    `/apps/my-projects/api/explore/?tab=${encodeURIComponent(tab)}`,
  );
...
export async function loadMe(): Promise<void> {
  ...
  const data = await hubGet("/apps/my-projects/api/me/");
  if (data?.success) content.innerHTML = data.html;
  content.style.opacity = "1";
}
...
export async function backToProjects(): Promise<void> {
  ...
  content.style.opacity = "0.5";
  pushDashboardUrl();
  const data = await hubGet("/apps/my-projects/api/projects-overview/");
  if (data?.success) content.innerHTML = data.html;
  content.style.opacity = "1";
}
...
export async function loadAccountSettings(): Promise<void> {
  ...
  if (!content) return;
  content.style.opacity = "0.5";
  const data = await hubGet("/apps/my-projects/api/account-settings/");
  if (data?.success) content.innerHTML = data.html;
  content.style.opacity = "1";
}
