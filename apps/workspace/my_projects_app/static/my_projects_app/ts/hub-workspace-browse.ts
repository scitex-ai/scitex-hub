...
export async function loadHubTabContent(
  ...
  const data = await hubGet(`/apps/my-projects/api/${tab}/${qs ? `?${qs}` : ""}`);
...
export async function loadHubBrowse(
  ...
  const data = await hubGet(
    `/apps/my-projects/api/browse/?path=${encodeURIComponent(path)}`,
  );
  if (data?.success) {
...
export async function loadHubFile(
  ...
  const data = await hubGet(
    `/apps/my-projects/api/file/?path=${encodeURIComponent(path)}`,
  );
  if (data?.success) target.innerHTML = data.html;
  target.style.opacity = "1";
}
