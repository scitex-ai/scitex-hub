...
export class UIManager {
  ...
  constructor(
    ...
    private nudgeCallback?: (
      direction: "up" | "down" | "left" | "right",
      shift: boolean,
    ) => void,
    private selectAllCallback?: () => void,
    private alignByAxisCallback?: (
    ...
    // Resizers now handled by unified resizer system via data-h-resizer attributes
  ...
  public updateStatusBar(message?: string): void {
    ...
    if (!message) {
      const zoomLevel =
        document.getElementById("canvas-zoom")?.textContent || "100%";
  ...
  // ========================================
  // PRIVATE - Panel Resizers
  // ========================================

  private initPanelResizers(): void {
    // All resizers handled by unified resizer system (data-h-resizer auto-init)
    console.log(
      "[UIManager] Panel resizers delegated to unified resizer via data attributes",
    );
  }
}
