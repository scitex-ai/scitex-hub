...
import type { DataTableManager } from "../_vis/ui/DataTabManager";
...
export class EditorCallbackHandlers {
   ...
   *
   * Enhanced to provide full multi-panel sync:
  ...
  public createSelectionCallback() {
    return async (obj: any) => {
      if (obj) {
        ...
        // Update status bar with panel info
        if (obj.isBundlePanel && obj.label) {
          const sizeInfo =
            obj.width && obj.height
              ? ` (${Math.round(obj.width)}×${Math.round(obj.height)} px)`
              : "";
          this.deps.updateStatusBar(`Panel ${obj.label} selected${sizeInfo}`);
        } else if (obj.name) {
          this.deps.updateStatusBar(`Object "${obj.name}" selected`);
        }
      } else {
        this.deps.propertiesManager.showNoSelection();
        this.deps.updateStatusBar("No selection");
      }
    };
  }
  ...
  public createElementSelectionCallback() {
    return (elementNames: string[], elementInfos: any[]) => {
      if (elementNames.length > 0 && elementInfos.length > 0) {
        ...
        console.log(
          `[VisEditor] Elements selected: ${elementNames.join(", ")}`,
          elementInfos,
        );

        // Show element properties in right panel
        this.deps.propertiesManager.showElementProperties(
          elementName,
          elementInfo,
        );
        ...
        for (let i = 0; i < elementNames.length; i++) {
          ...
          if (
            !csvCols &&
            info.element_type &&
            inferrableTypes.includes(info.element_type)
          ) {
        ...
        if (columnIndices.length > 0) {
          console.log(
            `[VisEditor] Highlighting columns: ${columnIndices.join(", ")}`,
          );
          ...
          if (elementNames.length === 1) {
            this.deps.updateStatusBar(
              `Element: ${elementInfo.label || elementName} (columns: ${colNamesStr})`,
            );
          } else {
            this.deps.updateStatusBar(
              `${elementNames.length} elements selected (columns: ${colNamesStr})`,
            );
          }
        } else {
          ...
          if (elementNames.length === 1) {
            this.deps.updateStatusBar(
              `Element: ${elementInfo.label || elementName}`,
            );
          } else {
            this.deps.updateStatusBar(
              `${elementNames.length} elements selected`,
            );
          }
        }
      } else {
  ...
  public createCanvasRestorationCallback(
    ...
    return async () => {
      ...
      if (activeTab?.canvasJson) {
        ...
        console.log(
          `[VisEditor] Restored canvas from tab: ${activeTab.figureName}`,
        );
      } else {
        ...
        if (savedState) {
          const restoredObjects =
            await this.deps.canvasManager.restoreCanvasContent();
          await loadMissingMetadata(restoredObjects);
          this.deps.updateRulersAreaTransform();
          console.log(
            `[VisEditor] Restored view: ${Math.round(this.deps.canvasManager.getCanvasZoomLevel() * 100)}%`,
          );
          ...
          this.deps.updateRulersAreaTransform();
          console.log(
            `[VisEditor] Initial zoom: ${Math.round(this.deps.canvasManager.getCanvasZoomLevel() * 100)}%`,
          );
        }
      }
      ...
      const savedCanvasTheme =
        localStorage.getItem("canvas-theme") || savedGlobalTheme;
      const canvasDarkMode = savedCanvasTheme === "dark";
      this.deps.canvasManager.updateCanvasTheme(canvasDarkMode);
      console.log(
        `[VisEditor] Canvas theme re-applied after restore: ${savedCanvasTheme}`,
      );
  ...
  public createPlotSelectCallback(dataTabManager: any) {
    return async (plot: any, category: string, csvData: string[][]) => {
      console.log(
        `[VisEditor] Gallery plot selected: ${plot.name} (${category})`,
      );
      ...
      if (projectContext.projectOwner && projectContext.projectSlug) {
        console.log(
          `[VisEditor] Creating pltz bundle for project: ${projectContext.projectOwner}/${projectContext.projectSlug}`,
        );
      } else {
        console.log(
          `[VisEditor] Creating pltz bundle in user's bundle directory (no project context)`,
        );
      }
