...
export class CsvDataCoordinator {
        ...
        try {
            ...
            for (const [category, info] of Object.entries(contents.categories || {})) {
                ...
                if (plot) {
                    ...
                    // Load CSV data
                    const csvUrl = plot.csv || `/apps/figrecipe/api/gallery/project/${category}/${plot.name}/csv/`;
                    try {
                    ...
                    if (!obj.axisMetadata) {
                        try {
                            const metaResponse = await fetch(`/apps/figrecipe/api/gallery/metadata/${category}/${plot.name}/`);
                            if (metaResponse.ok) {
    ...
    public async loadCsvForBundlePanel(obj: any): Promise<void> {
        ...
        try {
            const csvUrl = `/apps/figrecipe/api/bundles/pltz/data/?path=${encodeURIComponent(pltzPath)}`;
