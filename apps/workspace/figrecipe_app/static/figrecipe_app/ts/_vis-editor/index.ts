...
async function loadFigureFromDiskIfExists(
  ...
  try {
    ...
    const response = await fetch(
      `/apps/figrecipe/api/bundles/figz/load/?path=${encodeURIComponent(figzPath)}&project_owner=${encodeURIComponent(projectOwner)}&project_slug=${encodeURIComponent(projectSlug)}`,
    );
