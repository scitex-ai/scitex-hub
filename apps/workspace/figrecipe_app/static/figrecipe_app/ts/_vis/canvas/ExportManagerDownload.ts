...
export function downloadFigzBundle(
  ...
  const downloadUrl = `/apps/figrecipe/api/bundles/figz/download/?path=${encodeURIComponent(currentFigzPath)}`;
...
export function downloadPltzBundle(
  ...
  const downloadUrl = `/apps/figrecipe/api/bundles/pltz/download/?path=${encodeURIComponent(pltzPath)}`;
  triggerDownloadLink(downloadUrl, "");
