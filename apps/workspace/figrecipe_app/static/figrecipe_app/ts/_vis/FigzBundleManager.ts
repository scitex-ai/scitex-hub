...
// API endpoints
const API_BASE = "/apps/figrecipe/api/bundles/figz";
...
export interface CreateFigzBundleParams {
  ...
  panels?: Record<
    string,
    string | { spec: object; style: object; data_csv?: string }
  >;
  width_mm?: number;
  height_mm?: number;
  description?: string;
...
export class FigzBundleManager {
   ...
   */
  async getPanelPreviews(
    bundleId: string,
  ): Promise<Record<string, string | null>> {
    return this.fetchAPI<{ panels: Record<string, string | null> }>(
      `/${bundleId}/panels/`,
    ).then((r) => r.panels);
  }
