...
export class SciTeXEditor {
  ...
  constructor(
    ...
    this.containerEl = document.getElementById(
      options.containerId || "canvas-container",
    );
    this.previewEl = document.getElementById(
      options.previewId || "figure-preview",
    ) as HTMLImageElement;
    this.propertiesEl = document.getElementById(
      options.propertiesId || "dynamic-properties",
    );
  ...
  public async loadFigure(jsonPath: string, csvPath?: string): Promise<void> {
    ...
    try {
      const response = await fetch("/apps/figrecipe/api/editor/load/", {
        method: "POST",
        headers: {
  ...
  public async saveManualOverrides(): Promise<void> {
    ...
    try {
      const response = await fetch("/apps/figrecipe/api/editor/save/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
   ...
   */
  public async exportFigure(
    format: "png" | "pdf" | "svg" | "tiff" = "png",
    dpi: number = 300,
  ): Promise<void> {
    if (!this.jsonPath) {
      console.warn("[SciTeXEditor] No figure loaded");
    ...
    try {
      const response = await fetch("/apps/figrecipe/api/editor/export/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
