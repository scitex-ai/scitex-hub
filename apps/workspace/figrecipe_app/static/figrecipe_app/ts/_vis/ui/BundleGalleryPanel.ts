...
export class BundleGalleryPanel {
  ...
  private renderBundleCard(
    bundle: PltzBundleSummary | FigzBundleSummary,
  ): string {
            ...
            <div class="bundle-card" data-bundle-id="${bundle.id}" data-bundle-type="${isPltz ? "pltz" : "figz"}"
                ...
                <div class="bundle-card-preview">
                    <img src="${previewUrl}" alt="${bundle.name}"
                         onerror="this.src='/static/figrecipe_app/img/placeholder-plot.png'">
                </div>
    ...
    if (this.currentTab === "pltz") {
      ...
      if (this.selectedCategory !== "all") {
        bundles = bundles.filter(
          (b) => (b as PltzBundleSummary).category === this.selectedCategory,
        );
      }
    } else {
      ...
      if (this.selectedLayout !== "all") {
        bundles = bundles.filter(
          (b) => (b as FigzBundleSummary).layout === this.selectedLayout,
        );
      }
    }
  ...
  private attachEventListeners(): void {
    ...
    categoryFilter?.addEventListener("change", (e) => {
      this.selectedCategory = (e.target as HTMLSelectElement).value as
        | PltzCategory
        | "all";
      this.updateGrid();
    });
    ...
    layoutFilter?.addEventListener("change", (e) => {
      this.selectedLayout = (e.target as HTMLSelectElement).value as
        | FigzLayout
        | "all";
      this.updateGrid();
    });
    ...
    // Create button
    this.container
      .querySelector(".bundle-create-btn")
      ?.addEventListener("click", () => {
        this.showCreateDialog();
      });

    this.container
      .querySelector(".bundle-create-empty-btn")
      ?.addEventListener("click", () => {
        this.showCreateDialog();
      });
    ...
    // Refresh button
    this.container
      .querySelector(".bundle-refresh-btn")
      ?.addEventListener("click", () => {
        this.loadBundles();
      });
  ...
  private showCreateDialog(): void {
    ...
    const name = prompt(
      `Enter name for new ${this.currentTab === "pltz" ? "plot" : "figure"}:`,
    );
    if (!name) return;
