#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: /home/ywatanabe/proj/scitex-hub/apps/scholar_app/views/search/engines/core.py
...
from ..search_helpers import get_paper_authors, search_database_papers
    ...
    if SCITEX_SCHOLAR_AVAILABLE and source_list:
        ...
        if user_preferences:
            ...
            if missing_keys:
                ...
                logger.warning(
                    "   ℹ️ Add API keys at /scholar/api-keys/ for better performance"
                )
    else:
                ...
                if source == "arxiv":
                    ...
                    try:
                        semantic_results = search_semantic_scholar(
                            query, max_results=20, filters=filters
                        )
                        results.extend(semantic_results)
                        logger.info(
                            f"   ✓ Semantic Scholar returned {len(semantic_results)} results"
                        )
                    except Exception as e:
                        logger.warning(f"   ⚠️ Semantic Scholar search failed: {e}")
                elif source == "google_scholar":
                    logger.warning(
                        "   ⚠️ Google Scholar search disabled (not implemented)"
                    )
...
def search_with_scitex_scholar(
    ...
    try:
        ...
        if user and hasattr(user, "email") and user.email:
            ...
            logger.info("🚀 Using SciTeX-Scholar with user email for rate limits")
        else:
            logger.info("🚀 Using SciTeX-Scholar parallel search pipeline")
        ...
        pipeline = ScholarPipelineSearchParallel(
            max_workers=5, timeout_per_engine=180.0, use_cache=True, email=user_email
        )
        ...
        try:
            ...
            for paper in papers:
                ...
                result = {
                    ...
                    "citation_source": (
                        paper.get("source_engines", ["scitex"])[0]
