...
@shared_task(
    bind=True,
    name="apps.workspace.scholar_app.tasks.search_papers",
    max_retries=3,
    soft_time_limit=60,
    time_limit=90,
...
def search_papers(
    ...
    try:
        ...
        try:
            ...
            from apps.workspace.scholar_app.models import SearchIndex

            db_results = SearchIndex.objects.filter(title__icontains=query)[
                :max_results
            ]
            ...
            logger.info(
                f"DB search completed for user {user_id}: {len(results)} results"
            )
            return {
                "success": True,
                "user_id": user_id,
...
@shared_task(
    bind=True,
    name="apps.workspace.scholar_app.tasks.process_pdf",
    max_retries=2,
    soft_time_limit=120,
    time_limit=180,
    rate_limit="20/m",
...
@shared_task(
    bind=True,
    name="apps.workspace.scholar_app.tasks.batch_process_pdfs",
    soft_time_limit=600,
    time_limit=900,
...
def batch_process_pdfs(
    ...
    """
    logger.info(
        f"Batch PDF processing requested by user {user_id}: {len(pdf_paths)} files"
    )
...
@shared_task(
    bind=True,
    name="apps.workspace.scholar_app.tasks.fetch_paper_metadata",
    max_retries=3,
    soft_time_limit=30,
    time_limit=60,
    rate_limit="60/m",
...
def fetch_paper_metadata(
    ...
    """
    logger.info(
        f"Metadata fetch requested by user {user_id}: {identifier_type}:{identifier}"
    )
