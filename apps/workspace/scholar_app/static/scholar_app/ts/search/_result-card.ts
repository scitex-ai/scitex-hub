...
import { updateToolbarState } from "./_results-toolbar";
import { hideSearchLoading } from "./_search-loading";
 ...
 */
export function createResultCard(result: SearchResult): HTMLElement {
  ...
  const rankHint =
    rankReasons.length > 0 ? `Matches: ${rankReasons.join(" • ")}` : "";
