 ...
 */
export async function autoDownloadBibtexFile(url) {
    ...
    try {
        ...
        showAlert(`❌ <strong>Download failed</strong><br><br>` +
 ...
 */
export async function openAllPaperUrls(jobId) {
    if (!jobId) {
 ...
 */
function openUrlsInTabs(urls, jobId) {
    console.log(`[Open URLs] Opening ${urls.length} URLs as tabs...`);
    let openedCount = 0;
    urls.forEach((paper, index) => {
 ...
 */
export async function saveJobToProject(jobId) {
    console.log("[Save to Project] Job ID:", jobId);
    // Get current project ID from hidden input or use default
    const projectInput = document.getElementById("projectSelector");
    ...
    try {
        ...
        showAlert(`❌ Failed to save to project: ${error.message || "Unknown error"}`, "error");
    }
}
//# sourceMappingURL=job-actions.ts.map
