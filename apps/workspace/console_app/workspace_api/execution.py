...
def api_execute_script(request):
    ...
    try:
        ...
        # Path containment — component-wise, NOT a string prefix. A bare
        # ``str(file_full_path.resolve()).startswith(str(project_dir.resolve()))``
        # admits a sibling project (``<proj>-secret`` under the same jail) as
        # "inside"; the sink here is ``python <file>``, so a prefix escape runs
        # code from another of the user's projects. Component-wise containment
        # mirrors the file_read/file_write/file_create_delete sweep fix.
