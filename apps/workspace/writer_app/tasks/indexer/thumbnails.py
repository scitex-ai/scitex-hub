    ...
    try:
        ...
        if hasattr(project, "git_clone_path") and project.git_clone_path:
            ...
            if not hasattr(project, "owner") or not project.owner:
                logger.error(
                    f"[Thumbnail] Cannot determine user for project {project_id}"
                )
                return
            ...
            if not project_path:
                logger.error(
                    f"[Thumbnail] Project path not found for project {project_id}"
                )
                return
        ...
        if file_type in [".png", ".jpg", ".jpeg", ".tiff", ".tif"]:
            ...
            try:
                ...
                images = convert_from_path(
                    full_path, first_page=1, last_page=1, size=(200, 200)
                )
                if images:
                    images[0].save(thumb_path, "JPEG", quality=85)
            except ImportError:
                logger.warning(
                    "[Thumbnail] pdf2image not installed, skipping PDF thumbnail"
                )
                return
...
def generate_table_thumbnail(project_id, file_path):
    ...
    try:
        ...
        if hasattr(project, "git_clone_path") and project.git_clone_path:
            ...
            if not hasattr(project, "owner") or not project.owner:
                logger.error(
                    f"[TableThumbnail] Cannot determine user for project {project_id}"
                )
                return
            manager = get_project_filesystem_manager(project.owner)
            project_path = manager.get_project_root_path(project)
            if not project_path:
                logger.error(
                    f"[TableThumbnail] Project path not found for project {project_id}"
                )
                return
