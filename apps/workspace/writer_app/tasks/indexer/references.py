    ...
    try:
        ...
        if hasattr(project, "git_clone_path") and project.git_clone_path:
            ...
            if not hasattr(project, "owner") or not project.owner:
                logger.error(
                    f"[RefTracker] Cannot determine user for project {project_id}"
                )
                return
            ...
            if not project_path:
                logger.error(
                    f"[RefTracker] Project path not found for project {project_id}"
                )
                return
        ...
        for figure in figures:
            ...
            db.update_references(
                figure["id"], is_referenced=(ref_count > 0), reference_count=ref_count
            )
        ...
        logger.info(
            f"[RefTracker] Completed for project {project_id}: {references_updated} figures referenced"
        )

    except Exception as e:
        logger.error(
            f"[RefTracker] Error tracking references for project {project_id}: {e}"
        )
