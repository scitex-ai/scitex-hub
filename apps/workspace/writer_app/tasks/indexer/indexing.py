    ...
    try:
        ...
        if hasattr(project, "git_clone_path") and project.git_clone_path:
            ...
            if not hasattr(project, "owner") or not project.owner:
                logger.error(
                    f"[Indexer] Cannot determine user for project {project_id}"
                )
                return
            ...
            if not project_path:
                logger.error(
                    f"[Indexer] Project path not found for project {project_id}"
                )
                return
                    ...
                    try:
                        ...
                        if existing:
                            ...
                            if len(current_name) <= len(existing_name):
                                ...
                                logger.debug(
                                    f"[Indexer] Skipping duplicate: {current_name} (keeping {existing_name})"
                                )
                                skipped_count += 1
                                continue
                            else:
                                ...
                                logger.info(
                                    f"[Indexer] Replacing duplicate: {existing_name} -> {current_name}"
                                )
        ...
        logger.info(
            f"[Indexer] Completed for project {project_id}: {indexed_count} indexed, {skipped_count} skipped"
        )
...
def index_project_tables(project_id):
    ...
    try:
        ...
        if hasattr(project, "git_clone_path") and project.git_clone_path:
            ...
            if not hasattr(project, "owner") or not project.owner:
                logger.error(
                    f"[TableIndexer] Cannot determine user for project {project_id}"
                )
                return
            manager = get_project_filesystem_manager(project.owner)
            project_path = manager.get_project_root_path(project)
            if not project_path:
                logger.error(
                    f"[TableIndexer] Project path not found for project {project_id}"
                )
                return
            ...
            for file_path in project_path.glob(pattern):
                ...
                if file_path.suffix.lower() in SUPPORTED_TABLE_EXTENSIONS:
                    try:
                        ...
                        if db.check_if_indexed(
                            relative_path, file_hash, table_type="table"
                        ):
                            skipped_count += 1
                            continue  # Skip unchanged files
                        ...
                        if existing:
                            ...
                            if len(current_name) <= len(existing_name):
                                # Current file has shorter/same length name - skip it as duplicate
                                logger.debug(
                                    f"[TableIndexer] Skipping duplicate: {current_name} (keeping {existing_name})"
                                )
                                skipped_count += 1
                                continue
                            else:
                                # Current file has longer name - remove old entry and index new one
                                logger.info(
                                    f"[TableIndexer] Replacing duplicate: {existing_name} -> {current_name}"
                                )
        ...
        logger.info(
            f"[TableIndexer] Completed for project {project_id}: {indexed_count} indexed, {skipped_count} skipped"
        )
