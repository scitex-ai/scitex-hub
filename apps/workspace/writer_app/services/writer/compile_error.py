...
``views/editor/api/compilation.py`` — the FULL-compile path — needs the
same precedence, and that module must not have to import
``scitex``/``scitex_writer`` at module scope just to format an error
string.

Why it matters that both paths share this: until 2026-08-17 the full
compile built its result dict with NO ``error`` key at all, while the
front-end (``compilation-queue.ts::handleFailed``) reads
