...
def tables_api(request, project_id):
    ...
    try:
        ...
        logger.info(
            f"[Tables API] Returned {len(tables)} tables for project {project_id}"
        )
...
def refresh_tables_index(request, project_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Table indexing started in background"}
        )
...
def upload_tables(request, project_id):
    ...
    try:
        ...
        upload_dir = get_writer_workspace_path(project_path) / "uploads" / "tables"
        ...
        for uploaded_file in files:
            ...
            logger.info(
                f"[Upload] Saved table: {uploaded_file.name} ({uploaded_file.size} bytes)"
            )
        ...
        logger.info(
            f"[Upload] Uploaded {len(uploaded_files)} tables for project {project_id}"
        )
