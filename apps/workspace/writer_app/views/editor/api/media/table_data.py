...
def table_data_api(request, project_id, file_hash):
    ...
    try:
        ...
        if hasattr(project, "git_clone_path") and project.git_clone_path:
            ...
            from apps.infra.project_app.services.filesystem_manager import (
        ...
        logger.info(
            f"[Table Data API] Loaded {len(data)} rows × {len(columns)} cols from {table['file_name']}"
        )
...
def table_update_api(request, project_id, file_hash):
    ...
    try:
        ...
        logger.info(
            f"[Table Update API] Saved {len(data)} rows to {table['file_name']}"
        )
