...
def figures_api(request, project_id):
    ...
    try:
        ...
        if (
            request.GET.get("is_referenced")
            and request.GET.get("is_referenced") != "all"
        ):
        ...
        for fig in figures:
            if fig.get("thumbnail_path"):
                ...
                fig["thumbnail_url"] = (
                    f"/writer/api/project/{project_id}/thumbnail/{thumb_name}"
                )
            else:
                fig["thumbnail_url"] = "/static/images/thumbnail_placeholder.png"

        logger.info(
            f"[Figures API] Returned {len(figures)} figures for project {project_id}"
        )
...
def refresh_figures_index(request, project_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "Indexing started in background"}
        )
...
def upload_figures(request, project_id):
    ...
    try:
        ...
        for uploaded_file in files:
            ...
            logger.info(
                f"[Upload] Saved figure: {uploaded_file.name} ({uploaded_file.size} bytes)"
            )
        ...
        logger.info(
            f"[Upload] Uploaded {len(uploaded_files)} figures for project {project_id}"
        )
