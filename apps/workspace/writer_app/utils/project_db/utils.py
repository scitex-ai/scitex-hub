...
class DatabaseUtils:
    ...
    def check_if_indexed(
        ...
        with self.db.connection() as conn:
            cursor = conn.execute(
                f"SELECT file_hash FROM {table_name} WHERE file_path = ?", (file_path,)
            )
            row = cursor.fetchone()
            return row is not None and row["file_hash"] == file_hash

    def check_hash_exists(
        self, file_hash: str, table_type: str = "figure"
    ) -> dict | None:
        """
...
def get_project_db(project):
    ...
    """
    from apps.workspace.writer_app.utils.project_db.core import ProjectDatabase
    ...
    if hasattr(project, "git_clone_path") and project.git_clone_path:
        ...
        logger.debug(
            f"[ProjectDB] Using git_clone_path for project {project.id}: {project_path}"
        )
    else:
        ...
        try:
            ...
            if not project_path:
                raise ValueError(
                    f"Project path not found for project {project.id} (slug: {project.slug})"
                )

            logger.info(
                f"[ProjectDB] Using filesystem manager path for project {project.id}: {project_path}"
            )
        except Exception as e:
            logger.error(
                f"[ProjectDB] Could not determine project path: {e}", exc_info=True
            )
            raise ValueError(
