...
| 2 | **Git-history modal** (branch select + diff) | `templates/writer_app/index_partials/git_history_modal.html` | included from two **alternative** render paths (`writer_partial.html` and `index.html` — verified that neither includes the other, so this is NOT a duplicate-id risk; an earlier draft of this note said "can render twice" and that was wrong) |
...
| 4 | ~~Sidebar "Version History Timeline"~~ | `templates/writer_app/index_partials/sidebar.html` | **RETIRED — the file is gone.** It was dead code (`grep -rn "index_partials/sidebar.html"` found no `{% include %}`, no view, no URL and no JS fetch anywhere in this repo), and it was the only template carrying a Git/Version-History section, while the live `shared/_sidebar.html` has none. Removed, with a guard test so the duplicate cannot quietly return. Adjacent finding left for its own change: `id="init-writer-btn"` existed only in that file, while `static/writer_app/ts/modules/workspace-init.ts` still looks the id up — a JS hook wiring a button no live template provides. |
