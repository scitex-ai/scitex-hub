...
function copyToClipboard(elementId: string): void {
  ...
  navigator.clipboard
    ...
    .then(function () {
      ...
      button.innerHTML =
        '<svg aria-hidden="true" height="16" viewBox="0 0 16 16" width="16" fill="currentColor"><path d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0Z"></path></svg>';
...
function switchCloneMethod(button: HTMLElement): void {
  ...
  container
    .querySelectorAll(".clone-method-tab")
    .forEach((tab) => tab.classList.remove("active"));
  container
    .querySelectorAll(".clone-method-panel")
...
(function initDropdown() {
  ...
  // Initialize main tab switching (Local / Workspaces) — no data-bs-toggle to avoid Bootstrap closing the dropdown
  dropdownMenu
    .querySelectorAll("[data-clone-main-tab]")
    .forEach(function (tab) {
      tab.addEventListener("click", function (e: Event) {
        e.stopPropagation();
        const panelId = (tab as HTMLElement).dataset.cloneMainTab || "";
        ...
        const panel = dropdownMenu.querySelector(`#${panelId}`);
  ...
  console.log(
    "[CloneButton] Clone method tabs initialized:",
    dropdownMenu.querySelectorAll(".clone-method-tab").length,
  );
})();
