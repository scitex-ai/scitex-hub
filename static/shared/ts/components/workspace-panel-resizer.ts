...
export class WorkspacePanelResizer {
  ...
  /**
   * Initialize toggle button for a panel (restores state + attaches handler)
   */
  ...
  /**
   * Get validated expand width: rejects collapsed-size or oversized saved values
   */
  private getValidExpandWidth(
    config: PanelConfig,
    panel: HTMLElement,
  ): number | null {
    const saved = localStorage.getItem(this.storagePrefix + config.storageKey);
  ...
  private saveWidth(config: PanelConfig, width: number): void {
    try {
      localStorage.setItem(
        this.storagePrefix + config.storageKey,
        width.toString(),
      );
    } catch (e) {
  ...
  private restoreCollapseState(
    ...
    try {
      const isCollapsed =
        localStorage.getItem(config.collapseStorageKey) === "true";
      if (isCollapsed) {
  ...
  private initToggleClickHandler(config: PanelConfig): void {
    ...
    toggleBtn.addEventListener("click", (e) => {
      ...
      if (isCollapsed) {
        ...
        // Expanding - restore user's saved width or let CSS handle it
        const validWidth = this.getValidExpandWidth(config, targetPanel);
        if (validWidth) {
          // User has manually resized - restore their preference
          targetPanel.style.width = `${validWidth}px`;
          targetPanel.style.flexShrink = "0";
          targetPanel.style.flexGrow = "0";
        } else {
          // No valid saved width - clear inline styles so CSS takes over
          // (e.g., flex:1 panels like canvas-pane fill remaining space)
          targetPanel.style.width = "";
          targetPanel.style.maxWidth = "";
          targetPanel.style.flexShrink = "";
          targetPanel.style.flexGrow = "";
