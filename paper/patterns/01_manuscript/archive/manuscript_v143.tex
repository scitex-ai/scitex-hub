...
Modern scientific research requires consistent document preparation, reproducible computation, unified data management, and transparent figure provenance. Yet researchers often rely on fragmented tools: separate editors for manuscripts, external software for figures, ad-hoc scripts for analysis, and loosely maintained bibliography files. This fragmentation increases cognitive load and introduces irreproducibility at multiple stages. We present SciTeX, an AI-native, modular research automation ecosystem designed to overcome these barriers. SciTeX provides four integrated modules: Writer for structured LaTeX-based manuscript preparation with reproducible containerized builds; Scholar for literature management with metadata extraction and citation integration; Viz for publication-quality figure generation with programmable metadata; and Code for reproducible computation with local GPU, Apptainer, and HPC execution support. Each module operates independently while sharing a unified project structure, enabling seamless integration through lightweight symlinks and global identifiers. The system ensures cross-module traceability, allowing researchers to trace any figure, table, or result back to its generating code and input data. By combining modular design, reproducibility, and AI-native workflows, SciTeX reduces the activation energy required to produce high-quality scientific output while maintaining compatibility with traditional workflows. This manuscript itself was prepared within SciTeX, demonstrating the platform's self-descriptive capabilities.
...
%% -*- coding: utf-8 -*-
\begin{keyword}
reproducible research \sep scientific workflow \sep manuscript preparation \sep AI-native \sep modular architecture \sep provenance tracking
\end{keyword}
...
\subsection{Addressing Fragmented Workflows}
...
\subsection{AI-Native Design Philosophy}

SciTeX is designed for an era where AI assistants are integral to research workflows. Rather than treating AI as an afterthought, the architecture provides native integration points: structured APIs for code generation, metadata formats amenable to automated analysis, and workflow hooks for AI-assisted revision. This design enables researchers to collaborate effectively with LLMs and agents while maintaining scientific control. The AI augments but never overrides researcher intent---a crucial distinction as AI capabilities expand.

\subsection{Implications for Research Reproducibility}

The reproducibility crisis in science stems partly from inadequate infrastructure for capturing and communicating methodological details. SciTeX addresses this through:

\begin{itemize}
  \item Containerized compilation ensuring identical manuscript output across environments
  \item Figure metadata preserving exact generation parameters
  \item Execution provenance linking results to code and data
  \item Global identifiers enabling cross-module traceability
\end{itemize}

By embedding reproducibility infrastructure into everyday research tools, SciTeX makes rigorous practices the path of least resistance rather than an additional burden.
...
\begin{itemize}
  \item Automated figure revision using model-driven heuristic correction
  \item Integrated citation recommendation based on manuscript content
  \item Unified right-panel inspectors across all modules
  \item HPC-native execution pipelines with SLURM integration
  \item Automated manuscript consistency checking
  \item Complete provenance graph visualization
\end{itemize}
