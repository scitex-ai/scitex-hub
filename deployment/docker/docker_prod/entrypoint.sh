...
# ============================================
# Set SCITEX_HUB_BUILD_ID for CSS/JS cache busting
# ============================================
SCITEX_HUB_BUILD_ID=$(git -C /app rev-parse --short HEAD 2>/dev/null || date +%s)
export SCITEX_HUB_BUILD_ID
...
if [[ ! "$*" =~ "celery" ]]; then
    ...
    _init_visitor_pool() {
        ...
        echo_info "Background: reconciling visitor pool (quarantine + wipe+verify)..."
        ...
        echo_success "Background: visitor pool reconciled (only verified-clean slots distributable)"
    }
    _init_visitor_pool &
fi
