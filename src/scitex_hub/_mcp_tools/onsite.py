...
def register_onsite_tools(mcp) -> None:
    ...
    async def onsite_capture_page(
        ...
        """Use when the user asks to screenshot/capture/snapshot a SciTeX Hub workspace page or app iframe; drop-in replacement for hand-rolled Playwright page.screenshot() scripts pointed at the live SciTeX Hub workspace.
    ...
    async def onsite_check_permission(
        ...
        """Use when the user asks whether capture/automation is permitted for a SciTeX Hub project, or mentions the allow/deny/ask permission state; drop-in replacement for ad-hoc workspace-introspection scripts against the live SciTeX Hub workspace.
    ...
    async def onsite_get_context(page: str = "") -> str:
        """Use when the user asks for the current SciTeX Hub workspace context — username, active page, installed skills, available UI actions; drop-in replacement for hand-rolled Playwright introspection scripts pointed at the live SciTeX Hub workspace.
    ...
    async def onsite_eval_js(code: str, timeout: int = 10) -> str:
        """Use when the user asks to run/evaluate/execute JavaScript inside the SciTeX Hub workspace or app iframe; drop-in replacement for hand-rolled Playwright page.evaluate() scripts pointed at the live SciTeX Hub workspace.
    ...
    async def onsite_get_dev_app_url(project_id: str) -> str:
        """Use when the user asks for the workspace URL/path of a dev-installed SciTeX Hub app (needed before navigate/capture); drop-in replacement for hand-rolled workspace-introspection scripts against the live SciTeX Hub workspace.
    ...
    async def onsite_ui_action(steps: list, delay_ms: int = 900) -> str:
        """Use when the user asks to click/fill/navigate/scroll/highlight or otherwise drive the SciTeX Hub workspace UI; drop-in replacement for hand-rolled Playwright click/fill/goto scripts pointed at the live SciTeX Hub workspace.
