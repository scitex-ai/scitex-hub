...
class TestMobileScholar:
    ...
    def test_scholar_page_loads(self, authenticated_mobile_page, screenshot):
        """Scholar page loads on mobile."""
        resp = authenticated_mobile_page.goto("/apps/scholar/")
        screenshot(authenticated_mobile_page, "scholar_mobile_loaded")
        assert resp.status == 200, f"Scholar page returned {resp.status}"

    def test_scholar_results_scrollable(self, authenticated_mobile_page, screenshot):
        """Scholar search results container is vertically scrollable on mobile."""
        authenticated_mobile_page.goto("/apps/scholar/")
        ...
        results = authenticated_mobile_page.locator(
        ...
        screenshot(authenticated_mobile_page, "scholar_results_scrollable")

    def test_scholar_no_horizontal_overflow(self, authenticated_mobile_page):
