#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: tests/apps/workspace/public_projects_app/test_public_projects_single_pane.py
"""Public Projects (/apps/public-projects/) is one pane in the My Projects tree look.
...
class PublicProjectsSinglePaneTest(TestCase):
    ...
    def test_users_tab_counts_projects_not_repos(self):
        ...
        # Arrange
        url = "/apps/public-projects/api/explore/?tab=users"
        # Act
        response = self.client.get(url)
        # Assert
...
class PublicProjectsSignedOutTest(TestCase):
    ...
    def test_signed_out_public_projects_offers_no_sign_in_only_tabs(self):
        ...
        response = self.client.get(url)
        # Assert
        assert b'data-public-projects-tab="users"' not in response.content
