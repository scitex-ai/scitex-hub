...
class TestLoadFigzByPath:
    ...
    def test_missing_path_returns_400(self, request_factory, mock_user):
        ...
        from apps.workspace.figrecipe_app.views.api.bundles.path_figz import (
            load_figz_by_path,
        )
    ...
    def test_resolves_relative_path(self, request_factory, mock_user):
        ...
        with patch(
            "apps.workspace.figrecipe_app.views.api.bundles.path_figz.resolve_bundle_path"
        ) as mock_resolve:
            mock_resolve.return_value = Path("/data/projects/owner/slug/figure.figz")
            with patch(
                "apps.workspace.figrecipe_app.services.figz.FigzService.load_bundle"
            ) as mock_load:
...
class TestSaveFigzCanvas:
    ...
    def test_saves_canvas_successfully(self, request_factory, mock_user):
        ...
        with patch(
            "apps.workspace.figrecipe_app.services.figz.FigzService.save_canvas_as_bundle"
        ) as mock_save:
