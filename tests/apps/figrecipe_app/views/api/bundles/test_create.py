#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tests for apps/figrecipe_app/views/api/bundles/create.py"""

import pytest

# from apps.workspace.figrecipe_app.views.api.bundles.create import ...
...
# --------------------------------------------------------------------------------
# Start of Source Code from: apps/figrecipe_app/views/api/bundles/create.py
# --------------------------------------------------------------------------------
# """
...
#             return JsonResponse({"error": f"Project not found: {project_owner}/{project_slug}"}, status=404)
#         figures_dir = project_root / "scitex" / "vis" / "figures"
#     else:
#         figures_dir = Path(settings.MEDIA_ROOT) / "vis" / "bundles" / "figz" / str(request.user.id)
#
...
#
#     try:
#         from scitex.fig.io import save_figz_bundle
#         from scitex.io.bundle import zip_directory
#
#         figz_data = {
#             "spec": {
...
#         save_figz_bundle(figz_data, bundle_dir)
#
#         # ZIP-first architecture: Pack directory into .figz ZIP file
#         zip_path = figures_dir / f"{figure_name}.figz"
#         zip_directory(bundle_dir, zip_path)
#
#         logger.info(f"Created empty figz bundle: {zip_path} (from {bundle_dir})")
#
#         return JsonResponse({
#             "success": True,
#             "bundle_path": str(zip_path),
...
#         else:
#             return JsonResponse({"error": "No figz bundle found in project"}, status=404)
#     else:
...
# --------------------------------------------------------------------------------
# End of Source Code from: apps/figrecipe_app/views/api/bundles/create.py
# --------------------------------------------------------------------------------
