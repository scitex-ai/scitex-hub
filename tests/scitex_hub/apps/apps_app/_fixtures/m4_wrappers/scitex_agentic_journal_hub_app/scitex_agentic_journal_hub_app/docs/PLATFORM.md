# SciTeX Cloud App Platform Reference
...
# DataStore — CRUD
data.create("scitex_agentic_journal_hub_app", "Sample", {"name": "Test", "condition": "A"})
data.list_records("scitex_agentic_journal_hub_app", "Sample", filters={"condition": "A"})
data.get("scitex_agentic_journal_hub_app", "Sample", record_id)
data.update("scitex_agentic_journal_hub_app", "Sample", record_id, {"condition": "B"})
data.delete("scitex_agentic_journal_hub_app", "Sample", record_id)
data.search("scitex_agentic_journal_hub_app", "Sample", query="neural")

# FileVault — file storage
files.upload("scitex_agentic_journal_hub_app", "exports/data.csv", csv_content)
files.download("scitex_agentic_journal_hub_app", "exports/data.csv")
files.list_files("scitex_agentic_journal_hub_app", path="exports/")
files.delete("scitex_agentic_journal_hub_app", "exports/data.csv")

# JobQueue — background tasks
jobs.submit("scitex_agentic_journal_hub_app", "export_csv", params={"format": "xlsx"})
jobs.status("scitex_agentic_journal_hub_app", job_id)
jobs.list_jobs("scitex_agentic_journal_hub_app")
```
...
**CLI equivalent:**
```bash
scitex-cloud sdk data list scitex_agentic_journal_hub_app Sample
scitex-cloud sdk files upload scitex_agentic_journal_hub_app local.csv exports/data.csv
scitex-cloud sdk jobs submit scitex_agentic_journal_hub_app export_csv --params '{"format":"xlsx"}'
```
...
jobs:
  job_name:
    handler: scitex_agentic_journal_hub_app.jobs.job_name.execute
...
{% block content %}
    {% include "scitex_agentic_journal_hub_app/index_partial.html" %}
{% endblock %}
```
...
def index_view(request):
    ...
    context = build_scitex_agentic_journal_hub_app_context(request, project)
...
Scope ALL CSS under `.scitex_agentic_journal_hub_app-container` to prevent style leaks.
Use workspace CSS variables — no hardcoded colors.

```css
.scitex_agentic_journal_hub_app-container {
    --app-accent: var(--color-accent-emphasis, #4a90d9);
}
.scitex_agentic_journal_hub_app-container .my-card {
