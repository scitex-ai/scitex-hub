...
# DataStore — CRUD
data.create("scitex_live_paper_hub_app", "Sample", {"name": "Test", "condition": "A"})
data.list_records("scitex_live_paper_hub_app", "Sample", filters={"condition": "A"})
data.get("scitex_live_paper_hub_app", "Sample", record_id)
data.update("scitex_live_paper_hub_app", "Sample", record_id, {"condition": "B"})
data.delete("scitex_live_paper_hub_app", "Sample", record_id)
data.search("scitex_live_paper_hub_app", "Sample", query="neural")

# FileVault — file storage
files.upload("scitex_live_paper_hub_app", "exports/data.csv", csv_content)
files.download("scitex_live_paper_hub_app", "exports/data.csv")
files.list_files("scitex_live_paper_hub_app", path="exports/")
files.delete("scitex_live_paper_hub_app", "exports/data.csv")

# JobQueue — background tasks
jobs.submit("scitex_live_paper_hub_app", "export_csv", params={"format": "xlsx"})
jobs.status("scitex_live_paper_hub_app", job_id)
jobs.list_jobs("scitex_live_paper_hub_app")
```
...
**CLI equivalent:**
```bash
scitex-cloud sdk data list scitex_live_paper_hub_app Sample
scitex-cloud sdk files upload scitex_live_paper_hub_app local.csv exports/data.csv
scitex-cloud sdk jobs submit scitex_live_paper_hub_app export_csv --params '{"format":"xlsx"}'
```
...
```yaml
name: scitex_live_paper_hub_app
...
jobs:
  job_name:
    handler: scitex_live_paper_hub_app.jobs.job_name.execute
...
{% block content %}
    {% include "scitex_live_paper_hub_app/index_partial.html" %}
{% endblock %}
```

2. **`index_partial.html`** — AJAX partial (tab switching):
```html
<div class="scitex_live_paper_hub_app-container" data-pane-type="app">
...
def index_view(request):
    ...
    context = build_scitex_live_paper_hub_app_context(request, project)
...
Scope ALL CSS under `.scitex_live_paper_hub_app-container` to prevent style leaks.
Use workspace CSS variables — no hardcoded colors.

```css
.scitex_live_paper_hub_app-container {
    --app-accent: var(--color-accent-emphasis, #4a90d9);
}
.scitex_live_paper_hub_app-container .my-card {
