...
class TestDragResize:
    ...
    def test_drag_resizer_changes_width(
        ...
        assert (
            abs(width_change) > 20
        ), f"Width should change by at least 20px, got {width_change}px"

        show_step(
            page,
            5,
            5,
            f"Width changed: {initial_width}px → {final_width}px ({width_change:+d}px) ✓",
            "success",
        )
    ...
    def test_drag_respects_minimum_width(
        ...
        ), f"Width {final_width}px should be >= {min_width}px"
    ...
    def test_drag_collapsed_panel_expands(
        ...
        if not is_collapsed:
            show_test_result(
                page, False, "Could not collapse panel for test", delay_ms=2000
            )
        ...
        if not resizer_box:
            show_test_result(
                page, False, "Cannot get resizer bounding box", delay_ms=2000
            )
        ...
        is_still_collapsed = sidebar.evaluate(
        ...
        if is_still_collapsed:
            show_test_result(
                page, False, "Panel should expand when dragging", delay_ms=2000
            )
        assert (
            not is_still_collapsed
        ), "Panel should expand when dragging from collapsed state"
        assert (
            final_width > 100
        ), f"Panel should be wider than 100px, got {final_width}px"
        show_step(
            page, 6, 6, f"Collapsed panel expanded to {final_width}px ✓", "success"
        )
        show_test_result(
            page, True, "Collapsed panel expand test passed", delay_ms=2000
        )
