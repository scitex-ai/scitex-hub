...
| Value | Effect |
|-------|--------|
| `"left"` | Green bar on left edge of active item (frame-level, pointing inward) |
| `"right"` | Green bar on right edge of active item (in-app, pointing into content) |
| _(none)_ | Background highlight only, no edge bar |

Accent color uses `--workspace-icon-primary` (default: `#059669` emerald).
