...
def _add_input_boxes(d, C, CT):
    d.add_box("scripts_box", "Python Script", shape="document", emphasis="muted")
    d.add_box(
        "linter_box",
...
def _apply_transparent_bg(fig, ax):
    """Set transparent background so the diagram adapts to any theme."""
...
def main(
    ...
    # Transparent background — adapts to both light and dark themes
    d = _build_diagram(COLORS, CT)
    fig, ax = d.render(auto_fix=True)
    _apply_transparent_bg(fig, ax)
    stx.io.save(fig, CONFIG.SDIR_OUT / "workflow.svg", watermark=True)
