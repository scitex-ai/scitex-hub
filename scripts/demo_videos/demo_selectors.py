...
ID_SELECTOR = re.compile(r"^#([A-Za-z0-9_-]+)$")
DATA_SELECTOR = re.compile(r"\[(data-[a-z0-9-]+)(?:[~|^$*]?=([^\]]*))?\]")
...
def selector_tokens(selector: str) -> list[str]:
    ...
    An id is written literally in a template, so `#name` requires ``id="name"``.
    A data attribute is rendered from a value (``data-path="{{ file.path }}"``),
    so only the attribute is required to exist: the contract is that the elements
    carry the attribute, and which values exist is a runtime question the live
    check answers after the page is built.
    """
    tokens = []
    id_match = ID_SELECTOR.match(selector.strip())
    if id_match:
        tokens.append(f'id="{id_match.group(1)}"')
    for name, _value in DATA_SELECTOR.findall(selector):
        tokens.append(f"{name}=")
    return tokens


def is_semantic(selector: str) -> bool:
    """Whether the selector follows the spec: an id or a data attribute."""
    selector = selector.strip()
    return bool(ID_SELECTOR.match(selector)) or any(
        prefix in selector for prefix in SEMANTIC_PREFIXES
    )
